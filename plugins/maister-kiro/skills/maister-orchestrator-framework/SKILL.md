...
- **Phase Blocks**: Simple phase structure with inline transitions (`→ **CHAT GATE**`, `→ AUTO-CONTINUE`) — these are the only two transition types; see `orchestrator-patterns.md` § 2 for semantics
...
5. **Todo Progress**: Always track progress with todo/todo tools
