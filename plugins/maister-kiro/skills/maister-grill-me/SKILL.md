...
**Protocol parity**: Core grilling discipline is shared with `maister-grill-with-docs`; update both skills when changing one-question protocol or convergence rules.
...
If the user wants documentation maintained during grilling, suggest `maister-grill-with-docs` instead.
...
After confirmed shared understanding, the user may use `/maister-quick-plan`, `/maister-development`, or `maister-grill-with-docs` to harden vocabulary before building.
