...
- **Standalone mode**: If no state file, prompt user for each optional review using **CHAT GATE**.
...
6. **Set dependencies** using `todo` with `ordering in todo list`: "Compile report" blocked by ALL verification tasks above
...
1. **Check invocation context** for each optional review:
   ...
   - If orchestrator mode AND option is `false`: Skip (mark task as completed with `cancelled status`)
   - If orchestrator mode AND option is `null`: Warn and prompt user
   - If standalone mode: Prompt user with **CHAT GATE**

### Step 2: Set all tasks to in_progress

2. Use `todo` to set ALL enabled verification tasks to `status: "in_progress"`. For skipped optional reviews, use `todo` with `status: "completed"` and `metadata: {"skipped": true}`.
...
1. Use `todo` to set each verification task to `status: "completed"`
...
Use `todo` to set "Compile report" task to `status: "in_progress"`.
...
4. Use `todo` to set "Compile report" task to `status: "completed"`
