...
4. Present a summary to the user via **CHAT GATE** (present sequentially in chat; sequential single-choice):
...
- **Differs**: Show a brief diff summary and → **CHAT GATE** — Present the question in chat per file:
...
Invoke `docs-operator` subagent via subagent tool (subagent_type: `maister-docs-operator`):
...
- **If confident match** → → **CHAT GATE**: "This convention about [topic] fits [category/file]. Update it?" (Yes / Choose different / Cancel)
- **If ambiguous** → **CHAT GATE** listing possible categories/files + "Create new category" + "Create new file in [category]"
...
**Invoke `docs-operator` subagent** via subagent tool (subagent_type: `maister-docs-operator`) with context:
