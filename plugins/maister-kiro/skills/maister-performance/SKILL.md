...
The **CHAT GATE** text below is the adapter's `present_user_gate`, never a
...
**Execute**: Direct - invoke the shared engine for the verification options gate; **CHAT GATE** is only its adapter.
...
Invoke `verification-options` with sequential single-choice question "Which additional verification checks?" and exact options `["Code review", "Production readiness check"]`.
