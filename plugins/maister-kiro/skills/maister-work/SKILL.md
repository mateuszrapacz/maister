...
3. **Route to workflow** - Use `/maister-*` slash skill to invoke appropriate orchestrator skill
...
4. Present status to user with → **CHAT GATE**:
...
5. **Route using `/maister-*` slash skill:**

```
Use `/maister-*` slash skill:
...
3. **Route to appropriate workflow using `/maister-*` slash skill:**
...
→ **CHAT GATE** — Present the question in chat with options:
...
Then route to selected workflow using `/maister-*` slash skill.
```
...
The `/work` command delegates classification to the task-classifier subagent via subagent tool, which:
...
After classification/detection, this command routes to the appropriate orchestrator via `/maister-*` slash skill:
...
5. **Direct skill invocation** - Uses `/maister-*` slash skill for immediate orchestrator loading
