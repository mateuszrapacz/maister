...
3. **Check for task group items**: Call `todo list` to find existing task group items from the planner. If found, use them. If not, create them with `todo` for each task group (fallback for plans created before task system migration).
...
0. For every group in the wave, `todo` to `status: "in_progress"` with `owner: "maister-task-group-implementer"`.
...
3. **Wait for all wave members to return**, then for each result:
   ...
   - `todo` to `status: "completed"` with `metadata: {completed_at, tests_passed, files_modified, standards_applied, wave: N}`.
...
4. **Partial-wave failure handling**:
   ...
   - Mark successful groups in the wave as `completed` normally. Keep failed groups `in_progress` with `metadata: {failed_at, failure_reason, wave: N}` until the **CHAT GATE** recovery path resolves them.
...
2. If not complete, → **CHAT GATE** — Present the question in chat:
   ```
...
1. **Validate completion**:
   ...
   - All group tasks are `completed` via `todo list` (cross-validate against markdown checkboxes)
...
4. **→ **CHAT GATE** — Present the question in chat**:
   ```
...
3. If unclear: → **CHAT GATE** — Present the question in chat with options
