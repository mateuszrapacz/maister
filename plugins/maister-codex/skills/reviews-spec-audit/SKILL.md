...
**ACTION REQUIRED**: This command delegates to a different skill. The `<command-name>` tag refers to THIS command, not the target. Call the native subagent delegation with agent role="maister:spec-auditor" NOW. Pass the spec path in the prompt. Do not read files, explore code, or execute workflow steps yourself.
...
1. **Specification path**:
   ...
   - If not provided: Use plain-text user question to ask for spec.md path
...
**Invoke the spec-auditor agent NOW using the native subagent delegation:**
