...
- Only if no argument AND no bug context in session, use plain-text user question:
  ```
...
**Use the `native planning flow` tool to present the fix plan for user approval.**
...
### plan approval gate Gate: Mandatory Sections

**BLOCKING: Do NOT call `plan approval gate` until the plan file contains:**
...
If any section is missing, add it before calling plan approval gate.
...
- Use plain-text user question: "The reproduction test passes — the expected behavior already works under these conditions. Is the bug description accurate, or are there additional conditions?"
