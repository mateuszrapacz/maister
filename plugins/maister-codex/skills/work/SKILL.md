...
**NOTE**: This is a multi-step workflow that invokes the task-classifier subagent and orchestrator skills at specific steps. The `<command-name>` tag refers to THIS command only — you MUST still use the skill loader to invoke those other skills when instructed below. Follow ALL steps in order.
...
3. **Route to workflow** - Use skill loader to invoke appropriate orchestrator skill
...
4. Present status to user with plain-text user question:
...
3. **Route to appropriate workflow using skill loader:**
...
Use plain-text user question with options:
...
Then route to selected workflow using skill loader.
```
...
The `/work` command delegates classification to the task-classifier subagent via native subagent delegation, which:
...
After classification/detection, this command routes to the appropriate orchestrator via skill loader:
...
5. **Direct skill invocation** - Uses skill loader for immediate orchestrator loading
