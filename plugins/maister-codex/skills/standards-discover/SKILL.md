...
**Task Tracking**: At start of Phase 1, use `phase entries in orchestrator-state.yml` for all phases above (pending). Set dependencies: Phases 2-5 blocked by Phase 1 (they run in parallel after planning). Phase 6 blocked by Phases 2-5. Phases 7-9 sequential. At each phase start: `phase entries in orchestrator-state.yml` to `in_progress`. At each phase end: `phase entries in orchestrator-state.yml` to `completed`. For phases skipped due to scope (e.g., Phases 3-4 when `--scope=quick`), mark `completed` with `metadata: {skipped: true, reason: "scope=quick"}`.
...
5. **Get user confirmation** via plain-text user question before proceeding
...
**Step 4: Launch subagents in parallel** — Use the native subagent delegation with `agent role: default` for each phase.
...
- **High confidence (>= 80%)**: Use plain-text user question offering batch approval ("Apply all N high-confidence standards") or individual drill-down review. For drill-down, show full detail per finding: all evidence items with source attribution, examples (preferred/avoid), and confidence score breakdown (which factors contributed how many points).

- **Medium confidence (60-79%)**: Present each individually with full detail (evidence, examples, confidence breakdown). Use plain-text user question with Accept/Modify/Skip options per finding.

- **Low confidence (< threshold)**: Show the summary table rows only. Offer to expand details or skip all.

- **Conflicts**: Present each conflict showing both sides with their evidence and sources. Use plain-text user question to resolve (pick side A, pick side B, skip, or custom).
...
> **DELEGATION REQUIRED**: Do NOT write standard files directly using Write/Edit tools. ALL file operations MUST go through the `docs-operator` subagent (native subagent delegation).
>
> **SELF-CHECK before each file operation**: "Am I about to write a file directly? STOP — invoke docs-operator via native subagent delegation instead."
...
3. **Invoke `docs-operator` subagent** via native subagent delegation (agent role: `native Codex subagent`) — Pass prepared content. For creates: new file. For updates: merge new findings with existing. Wait for completion, then continue with the next standard.
4. **After all standards applied, invoke `docs-operator` subagent** via native subagent delegation to regenerate INDEX.md. Wait for completion, then continue with step 5.
5. **Invoke `docs-operator` subagent** via native subagent delegation to verify AGENTS.md integration — ensure standards directory is referenced. Wait for completion, then display the application summary.
