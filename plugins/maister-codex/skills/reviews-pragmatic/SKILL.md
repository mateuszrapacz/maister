...
**ACTION REQUIRED**: This command delegates to a different skill. The `<command-name>` tag refers to THIS command, not the target. Call the native subagent delegation with agent role="maister:code-quality-pragmatist" NOW. Pass the path to analyze in the prompt. Do not read files, explore code, or execute workflow steps yourself.
...
1. **Path to analyze**:
   ...
   - If not provided: Use plain-text user question to ask what to analyze (file, directory, or task path)

## Your Instructions

**Invoke the code-quality-pragmatist agent NOW using the native subagent delegation:**
