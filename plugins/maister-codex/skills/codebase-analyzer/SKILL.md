...
**3c. Launch agents** — Use the native subagent delegation with `agent role="Explore"` — one call per selected role, all in ONE message.
...
After all Explore agents complete, delegate to `codebase-analysis-reporter` subagent via native subagent delegation:
