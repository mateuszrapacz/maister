...
If the question is **"how should I implement X?"** — this is a modeling skill, not an implementation skill. Use `maister:problem-classifier` or an archetype mapper instead.
...
After presenting the linguistic analysis, ask about unresolved ambiguities and uncertain generalizations. Use `plain-text user question` (up to 4 questions per call).
...
2. If affects boundary placement or generalization scope: **stop and ask** using `plain-text user question`.
