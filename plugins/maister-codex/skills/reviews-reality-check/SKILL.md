...
**ACTION REQUIRED**: This command delegates to a different skill. The `<command-name>` tag refers to THIS command, not the target. Call the native subagent delegation with agent role="maister:reality-assessor" NOW. Pass the task path in the prompt. Do not read files, explore code, or execute workflow steps yourself.
...
1. **Task path**:
   ...
   - If not provided: Use plain-text user question to ask for task path

## Your Instructions

**Invoke the reality-assessor agent NOW using the native subagent delegation:**
