...
4. Present a summary to the user via plain-text user question (multi-select):
...
- **Differs**: Show a brief diff summary and use plain-text user question per file:
...
Invoke `docs-operator` subagent via native subagent delegation (agent role: `native Codex subagent`):
...
- **If confident match** → plain-text user question: "This convention about [topic] fits [category/file]. Update it?" (Yes / Choose different / Cancel)
- **If ambiguous** → plain-text user question listing possible categories/files + "Create new category" + "Create new file in [category]"
...
**Invoke `docs-operator` subagent** via native subagent delegation (agent role: `native Codex subagent`) with context:
