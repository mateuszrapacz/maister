...
**ACTION REQUIRED**: This command delegates to a subagent. The `<command-name>` tag refers to THIS command, not the target. Invoke the production-readiness-checker subagent via the native subagent delegation NOW. Pass path and target arguments. Do not read files, explore code, or execute workflow steps yourself.
...
1. **Path to analyze**:
   ...
   - If not provided: Use plain-text user question to ask what to check
...
**Invoke the production-readiness-checker subagent NOW using the native subagent delegation:**
...
Use native subagent delegation:
  ...
  prompt: |
    Verify production readiness at: [path from user or from plain-text user question]
