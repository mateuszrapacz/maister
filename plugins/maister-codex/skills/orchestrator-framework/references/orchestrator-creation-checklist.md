...
- [ ] **POST-CONTINUATION blocks** — After skill loader phases, explicit instructions to read state, update completed_phases, and continue
...
- [ ] **Decision gates** — Phases receiving `decisions_needed` present to user via plain-text user question
- [ ] **Interactive mode** — `plain-text user question` at every `→ Pause` transition
- [ ] **Standards discovery** — `.maister/docs/INDEX.md` referenced in spec, plan, implement, verify phases
- [ ] **phase entries in orchestrator-state.yml initialization** — Tasks created for all phases at workflow start with `addBlockedBy` dependencies
...
| Implicit user prompts without plain-text user question | User loses control |
| Inline STOP reminders at END of phases | Easily missed; use `→ Pause` transitions instead |
| Vague subagent calls ("invoke X") | Must show explicit Skill/native subagent delegation parameters |
...
| Auto-accepting subagent decisions | User must consent via plain-text user question |
