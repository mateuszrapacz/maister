...
**Always use Skill/native subagent delegations to delegate. Never execute delegated work inline.**

When a phase requires delegation:
1. Use the **skill loader** for **skills** — loads SKILL.md instructions into the main agent's context; the main agent executes the skill's instructions and continues with the orchestrator workflow afterward
2. Use the **native subagent delegation** for **subagents/agents** — spawns an isolated subprocess that returns results when complete
3. Wait for completion before continuing

**Skills and agents are NOT interchangeable.** Skills always use skill loader; agents always use native subagent delegation. Never invoke a skill via native subagent delegation (`agent role`) — it will fail with "Agent type not found."

**Why skills MUST use skill loader**: Skills like `codebase-analyzer`, `implementation-plan-executor`, and `implementation-verifier` spawn their own subagents (Explore agents, reporters, planners). Subagents cannot spawn other subagents — so these skills must run in the main agent context via skill loader.

**Companion agent pattern** (e.g., `docs-operator`): Only works for skills that do NOT spawn subagents (like `docs-manager` which only does file operations). A companion agent preloads the skill via the `skills` frontmatter field and is invoked via native subagent delegation. This pattern fails for any skill that needs to spawn subagents.
...
| Reading a SKILL.md then doing the work | Skill files are instructions FOR skills | Use skill loader to invoke |
...
1. **Clarifying questions phases** — plain-text user question is direct
...
**`→ Pause` means STOP and USE plain-text user question.** This is NOT optional. You MUST invoke the `plain-text user question` tool and WAIT for user response. Proceeding without it is a protocol violation.

All orchestrators pause at `→ Pause` transitions for user review and prompt for optional phases.

**State ordering rule**: Phase state MUST NOT be updated to 'completed' (via orchestrator-state.yml or phase entries in orchestrator-state.yml) until AFTER the user responds to the exit gate. Correct sequence: finish phase work → call plain-text user question → receive user response → update state to completed.
...
You MUST invoke `plain-text user question` at every `→ Pause` in every permission mode — `default`, `acceptEdits`, `auto`, `plan`, `bypassPermissions`. "Minimize clarifying questions" applies to the model's own discretionary asks; it does not override workflow checkpoints the orchestrator has been instructed to enforce.
...
- No permission mode, session-reminder, prior-session pattern, or "this task is simple" judgment exempts you from firing `plain-text user question` at `→ Pause`.
...
- Do NOT use plain-text user question
...
|--------------|----------------|
| Proceeding without plain-text user question at phase gates | User loses control, can't review or stop |
...
| Marking phase as completed (state/phase entries in orchestrator-state.yml) before the exit gate executes | State corruption — downstream phases see false "completed" status. Gate → user response → state update. Never reverse this order. |
...
When a subagent returns `decisions_needed` items, the orchestrator MUST present them to the user via plain-text user question. Decisions are never silently skipped.
...
orchestrator:
  ...
  # Task tracking IDs (maps phase names to phase entries in orchestrator-state.yml IDs)
  task_ids:
    phase-1: null
    phase-2: null
...
8. **Create task items**: `phase entries in orchestrator-state.yml` for all phases, then `phase entries in orchestrator-state.yml addBlockedBy` for dependencies. On resume, also restore completed phase statuses.
...
1. Create all phase tasks (same `phase entries in orchestrator-state.yml` loop, all start pending)
2. Set dependencies (same `phase entries in orchestrator-state.yml addBlockedBy`)
3. Mark completed phases (`phase entries in orchestrator-state.yml` to `completed` with `metadata: {restored: true}`)
...
If prerequisites missing, use plain-text user question: "Start from Phase 1", "Specify different phase", or "Exit".
...
2. For each issue: trivial/auto-fixable → fix silently, log action; non-trivial → plain-text user question
