...
**INVOKE NOW**: Use native subagent delegation with `agent role: native Codex subagent`
...
For each category in strategy:
  Use native subagent delegation: source_category=[category_id] → analysis/findings/[prefix]-*.md
```
...
> **ANTI-PATTERN**: Do NOT present all decision areas in a single summary table and ask one combined "do you agree?" question. Each area MUST get its own detailed presentation and its own plain-text user question call.
>
