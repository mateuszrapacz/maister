...
- `maister:problem-classifier` if the problem class is unclear
...
Use `plain-text user question` only if the fit is genuinely ambiguous (e.g., unclear whether single-user or multi-user access):
...
```
plain-text user question (one question per suspicious pair, up to 4 per call):
