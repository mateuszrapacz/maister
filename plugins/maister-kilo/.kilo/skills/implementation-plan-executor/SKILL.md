...
4. **Partial-wave failure handling**:
   ...
   - Mark successful groups in the wave as `completed` normally. Keep failed groups `in_progress` with `metadata: {failed_at, failure_reason, wave: N}` until the → **CHAT GATE** — Present the question in chat and wait for user response recovery path resolves them.
...
2. If not complete, use → **CHAT GATE** — Present the question in chat and wait for user response:
   ```
...
4. **Use → **CHAT GATE** — Present the question in chat and wait for user response**:
   ```
...
3. If unclear: use → **CHAT GATE** — Present the question in chat and wait for user response with options
