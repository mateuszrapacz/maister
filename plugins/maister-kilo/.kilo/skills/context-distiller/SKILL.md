...
Do NOT invoke when the user asks how to implement a specific feature, requests code changes, needs deployment or technology decisions, or needs problem-class classification without generalization analysis.
...
Options:
- **English** — all questions, reports, and maps in English
- **Polish** — all questions, reports, and maps in Polish (preserves pedagogical PL/EN rubric examples)
...
If the question is **"how should I implement X?"** — this is a modeling skill, not an implementation skill. Use `problem-classifier` or an archetype mapper instead.
...
After presenting the linguistic analysis, ask about unresolved ambiguities and uncertain generalizations. Use `→ **CHAT GATE** — Present the question in chat and wait for user response` (up to 4 questions per call).
...
2. If affects boundary placement or generalization scope: **stop and ask** using `→ **CHAT GATE** — Present the question in chat and wait for user response`.
...
After producing the distillation map, hand off based on what the analysis revealed:
