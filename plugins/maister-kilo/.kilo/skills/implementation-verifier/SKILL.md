...
- **Standalone mode**: If no state file, prompt user for each optional review using → **CHAT GATE** — Present the question in chat and wait for user response.
...
1. **Check invocation context** for each optional review:
   ...
   - If standalone mode: Prompt user with → **CHAT GATE** — Present the question in chat and wait for user response
