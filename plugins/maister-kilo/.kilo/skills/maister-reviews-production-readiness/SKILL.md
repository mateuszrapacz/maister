...
1. **Path to analyze**:
   ...
   - If not provided: Use → **CHAT GATE** — Present the question in chat and wait for user response to ask what to check
...
Use Task tool:
  ...
  prompt: |
    Verify production readiness at: [path from user or from → **CHAT GATE** — Present the question in chat and wait for user response]
