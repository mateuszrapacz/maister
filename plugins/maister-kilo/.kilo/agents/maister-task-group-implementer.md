---
description: "Execute a single task group from an implementation plan with continuous standards discovery. Writes code, runs tests, returns structured execution report. Does NOT mark checkboxes - main agent handles"
mode: subagent
permission:
  edit: allow
  bash: ask
...
Can't proceed due to missing dependency or unclear spec → Report FAILED with clear explanation → Main agent will use → **CHAT GATE** — Present the question in chat and wait for user response to decide path forward
