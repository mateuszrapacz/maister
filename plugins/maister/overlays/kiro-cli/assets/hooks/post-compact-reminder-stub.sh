#!/bin/bash
# STUB: Kiro CLI has no preCompact hook equivalent; the gap is documented by the host contract.
# Not wired in agents/maister.json — manual/orchestrator guidance only.
# Adapted from the shared post-compact reminder for future parity if Kiro adds compaction hooks.
