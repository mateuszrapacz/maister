...
remove_created_native-host_directories() {
  local agents native-host
  agents=$1 native-host=$2
  test "$agents" != yes || rmdir "$3" 2>/dev/null || true
  test "$native-host" != yes || rmdir "$(dirname "$3")" 2>/dev/null || true
}
...
effective_summary() {
  ...
  awk -v host="$host" -v action="$action" '
    ...
    END {
      ...
      printf "host-native TOML action: %s\n", action
...
run_init_transaction() {
  ...
  local toml_tombstone toml_action native-host_created agents_created rollback_failed
  local critical_recovery
  config=$1 host=$2 can_ask=$3 answer=$4 template=$5
  shift 5
  case "$host" in native-host|non-native-host) ;; *) die "host signal must be native-host or non-native-host" ;; esac
  ...
  toml_action=not-applicable native-host_created=no agents_created=no
  if test "$host" = native-host; then
    toml="$directory/../.host-config/agents/advisor.toml"
  ...
  if test "$host" = native-host; then
    if test "${MAISTER_ADVISOR_FAIL:-}" = second-action; then second_status=1
      ...
      if test -e "$toml" && cmp -s "$toml" "$toml_candidate"; then
        ...
        if test ! -d "$(dirname "$(dirname "$toml")")"; then
          if mkdir "$(dirname "$(dirname "$toml")")"; then native-host_created=yes; else second_status=1; fi
        fi
    ...
    if test "$second_status" -ne 0; then
      ...
      remove_created_native-host_directories "$agents_created" "$native-host_created" "$(dirname "$toml")"
...
case "$command" in
  ...
  toml-candidate)
    ...
    ;;
  init)
    test "$#" -ge 6 || die "usage: init CONFIG native-host|non-native-host yes|no ANSWER|- TEMPLATE [--advisor=on|off ...]"
