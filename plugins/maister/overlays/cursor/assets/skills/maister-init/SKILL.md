...
authoritative `native-host` or `non-native-host` signal. Only the generated host-native adapter
passes `native-host`; never infer it from `.host-config/`, an existing TOML file, or the
environment. The pre-flight validates and stages all managed artifacts, then
commits `.maister/config.yml` and, for host-native, `.host-config/agents/advisor.toml` as
one transaction. A host-native `on` creates or field-reconciles the strict current
TOML; `off` atomically removes it. A non-host-native invocation never inspects or
touches that file.
...
settings, managed-field removals, host-native TOML outcome, and capability-matrix
...
pre-flight transaction, including the generated host-native adapter's bundled TOML
