...
4. Present a summary to the user via AskQuestion (multi-select):
...
- **Differs**: Show a brief diff summary and use AskQuestion per file:
...
- **If confident match** → AskQuestion: "This convention about [topic] fits [category/file]. Update it?" (Yes / Choose different / Cancel)
- **If ambiguous** → AskQuestion listing possible categories/files + "Create new category" + "Create new file in [category]"
...
For **creates**:
> "Create documentation file: standards/[category]/[name].md. Category: [category]. Content: [conventions]. Create with proper markdown, organized sections, code examples. Add to INDEX.md with practice-specific description. Verify project-instruction integration."
...
3. Verify project-instruction integration
