...
**Probing questions** — when triggered, ask using `AskQuestion`. Ask 2-3 at a time, not all at once. Use answers to build up the reformulated requirement iteratively.
...
**Process**:
1. Ask the first 2-3 probing questions via `AskQuestion`
2. Based on answers, draft a reformulated requirement that describes **observable behavior** instead of status changes
3. Present the draft to the user via `AskQuestion` with options: "Akceptuję", "Chcę doprecyzować" (+ free text)
...
3. Use `AskQuestion` to ask the most relevant 2-3 questions from the matched cluster
...
3. Present them and ask using `AskQuestion`: *"Is any of these scenarios possible in your domain?"*
