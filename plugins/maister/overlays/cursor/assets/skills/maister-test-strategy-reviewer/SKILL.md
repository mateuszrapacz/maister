...
After forming a preliminary classification, **always present it to the user** via `AskQuestion` before proceeding. The user may know things that aren't visible in the code:
...
**Before diagnosing — ask about exceptions** via `AskQuestion`:
...
Tests can live at the aggregate level OR at the application service / facade level. Before recommending, **ask the user** via `AskQuestion`:
