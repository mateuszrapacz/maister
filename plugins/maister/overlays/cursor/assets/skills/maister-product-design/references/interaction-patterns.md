...
```
Present complete draft → AskQuestion (approve / change / rethink / add detail / explain)
  → [revision] → present complete revised draft → AskQuestion (same options)
  → [after soft cap] → AskQuestion (approve current / one more revision / step back)
```
...
Rich contextual prose BETWEEN AskQuestion calls is the actual design conversation. AskQuestion calls are punctuation marks -- they structure the conversation but do not replace it.
