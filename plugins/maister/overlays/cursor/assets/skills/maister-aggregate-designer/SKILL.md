...
Use `AskQuestion` only if the fit is genuinely ambiguous (e.g., unclear whether single-user or multi-user access):
...
```
AskQuestion (one question per suspicious pair, up to 4 per call):
