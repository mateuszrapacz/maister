...
const SAFE_COMMAND =
	/^[A-Za-z0-9][A-Za-z0-9_./:+,=@%+-]*(?: [A-Za-z0-9_./:+,=@%+-]+)*$/u;
...
function inputPath(
	value,
	name,
	{ required = true, mustExist = false, regularFile = false } = {},
) {
	...
	if (/[;`$<>{}|&]/u.test(raw)) {
		fail(
			"E_RELEASE_INTERFACE_PATH",
			`${name} contains shell or JavaScript delimiter characters`,
			{ name },
		);
	}
	...
	if (mustExist) {
		...
		try {
			...
			fail("E_RELEASE_INTERFACE_PATH", `${name} does not exist`, {
				name,
				path: resolved,
		...
		if (regularFile && (!stat.isFile() || stat.isSymbolicLink())) {
			fail("E_RELEASE_INTERFACE_PATH", `${name} must be a regular file`, {
				name,
				path: resolved,
			});
		}
...
function targetValue(value) {
	if (!SUPPORTED_TARGET_IDS.includes(value)) {
		fail(
			"E_RELEASE_INTERFACE_TARGET",
			`TARGET must be one of ${SUPPORTED_TARGET_IDS.join(", ")}`,
			{ target: value },
		);
	}
	return value;
...
function timestampValue(value) {
	...
	if (!ISO_TIMESTAMP.test(normalized) || Number.isNaN(Date.parse(normalized))) {
		fail(
			"E_RELEASE_INTERFACE_TIMESTAMP",
			"timestamp must be an ISO-8601 UTC value with milliseconds",
			{ value },
		);
	}
	return normalized;
...
function copySourceTree(stage, target) {
	...
	for (const entry of [
		...
		fs.cpSync(
			path.join(sourcePluginRoot, entry),
			path.join(stagePluginRoot, entry),
			{
				recursive: true,
	...
	fs.copyFileSync(
		path.join(sourcePluginRoot, "agent-projection-v1.json"),
		path.join(stagePluginRoot, "agent-projection-v1.json"),
	);
	fs.cpSync(
		path.join(sourcePluginRoot, "overlays", target),
		path.join(stagePluginRoot, "overlays", target),
		{
			recursive: true,
			preserveTimestamps: true,
		},
	);
	for (const otherTarget of SUPPORTED_TARGET_IDS.filter(
		...
		for (const name of ["overlay.yml", "inventory.yml"]) {
			fs.copyFileSync(
				path.join(sourceOverlay, name),
				path.join(stagedOverlay, name),
			);
		}
	}
}
...
function assertPackageClosure(stage, target) {
	...
	for (const relative of required) {
		...
		if (!fs.existsSync(candidate) || fs.lstatSync(candidate).isSymbolicLink()) {
			fail(
				"E_RELEASE_INTERFACE_PACKAGE",
				"package source closure is incomplete",
	...
	const overlays = fs
		.readdirSync(path.join(pluginRoot, "overlays"), { withFileTypes: true })
		.filter((entry) => entry.isDirectory())
		.map((entry) => entry.name)
		.sort();
	if (
		JSON.stringify(overlays) !==
		JSON.stringify([...SUPPORTED_TARGET_IDS].sort())
	) {
		fail(
			"E_RELEASE_INTERFACE_PACKAGE",
			"package must contain every projection-contract overlay",
			{ target, overlays },
		);
	}
	...
	for (const otherTarget of SUPPORTED_TARGET_IDS.filter(
		...
		if (
			fs.existsSync(path.join(pluginRoot, "overlays", otherTarget, "assets"))
		) {
			fail(
				"E_RELEASE_INTERFACE_PACKAGE",
				"package contains foreign target assets",
				{ target, path: `overlays/${otherTarget}/assets` },
			);
		}
	}
	...
	for (const relative of forbidden) {
		if (fs.existsSync(path.join(pluginRoot, relative))) {
			fail(
				"E_RELEASE_INTERFACE_PACKAGE",
				"package contains forbidden legacy or behavior-bearing assets",
				{ target, path: relative },
			);
		}
	}
	...
	const visit = (current, relative = "") => {
		for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
			...
			if (
				forbiddenSegments.has(entry.name) ||
				/(?:^|\/)pi-subagents(?:\/|$)/u.test(childRelative)
			) {
	...
	if (forbiddenEntries.length > 0) {
		fail(
			"E_RELEASE_INTERFACE_PACKAGE",
			"package contains Pi operator-owned state or an external prerequisite",
			{
				target,
...
function removeForbiddenPackageTrees(stage) {
	...
	for (const relative of [
		...
		fs.rmSync(path.join(pluginRoot, relative), {
			recursive: true,
			force: true,
		});
	}
...
function validateAndEmbedAttestation(
	stage,
	sourceCommit,
	sourceVersion,
	epoch,
	...
	if (attestation.source_commit !== sourceCommit) {
		fail(
			"E_RELEASE_INTERFACE_E3",
			"E3 source commit does not match package source",
			{ expected: sourceCommit, actual: attestation.source_commit },
		);
	}
	if (attestation.source_version !== sourceVersion) {
		fail(
			"E_RELEASE_INTERFACE_E3",
			"E3 source version does not match package source",
			{ expected: sourceVersion, actual: attestation.source_version },
		);
	}
	const coreHash = portableCoreTreeHash(stage);
	if (
		attestation.portable_core_tree_hash !== coreHash ||
		attestation.artifact_digest !== coreHash
	) {
		fail(
			"E_RELEASE_INTERFACE_E3",
			"E3 portable-core digest does not match package source",
			{
				expected: coreHash,
...
function writeArchive(stage, archivePath, fileListPath) {
	fs.writeFileSync(
		fileListPath,
		`${sortedArchiveEntries(path.join(stage, "plugins/maister")).join("\n")}\n`,
	);
...
export function packageTarget({ env = process.env } = {}) {
	...
	try {
		...
		if (
			...
			fs.statSync(archivePath).size === 0
		) {
			fail("E_RELEASE_INTERFACE_PACKAGE", "package archive was not created", {
				archivePath,
			});
		}
...
function sourceCommitValueFrom(env) {
	const value = boundedText(env.SOURCE_COMMIT, "SOURCE_COMMIT", {
		max: 64,
	}).toLowerCase();
	if (!FULL_COMMIT.test(value))
		fail(
			"E_RELEASE_INTERFACE_COMMIT",
...
function sourceVersionValueFrom(env, { allowUnknown = true } = {}) {
	...
	if (
		...
		fail(
			"E_RELEASE_INTERFACE_VERSION",
			"SOURCE_VERSION must be a bounded release version",
			{ value },
		);
	}
	if (!VERSION.test(value) || (!allowUnknown && value === "unknown")) {
		fail(
			"E_RELEASE_INTERFACE_VERSION",
			"SOURCE_VERSION must be a safe release version",
			{ value },
		);
	}
	return value;
...
function epochValueFrom(env) {
	const value = boundedText(env.SOURCE_DATE_EPOCH, "SOURCE_DATE_EPOCH", {
		max: 20,
	});
	if (!/^\d+$/u.test(value))
		fail(
			"E_RELEASE_INTERFACE_EPOCH",
	...
	if (
		...
		new Date(epoch * 1000).toString() === "Invalid Date"
	) {
		fail(
			"E_RELEASE_INTERFACE_EPOCH",
...
function commandValueFrom(env) {
	...
	if (!SAFE_COMMAND.test(value)) {
		fail(
			"E_RELEASE_INTERFACE_COMMAND",
			"E3_TEST_COMMAND must contain only safe command tokens separated by spaces",
			{ value },
		);
	}
	return value;
...
function runGenerateE3({ env = process.env } = {}) {
	const output = inputPath(
		env.E3_OUTPUT || path.join(env.DIST_DIR || "dist", "e3-portable-core.json"),
		"E3_OUTPUT",
	);
	const result = boundedText(env.E3_RESULT, "RESULT", { max: 16 });
	if (result !== "passed")
		fail(
			"E_RELEASE_INTERFACE_RESULT",
			"E3_RESULT must be passed after test-core",
			{ result },
		);
	const expiresAt = env.E3_EXPIRES_AT
		? timestampValue(env.E3_EXPIRES_AT)
		: undefined;
	const scenario = boundedText(
		env.E3_SCENARIO || "portable-core-v1",
		"SCENARIO",
		{ max: 256 },
	);
	const scenarioVersion = boundedText(
		env.E3_SCENARIO_VERSION || "1.0.0",
		"SCENARIO_VERSION",
		{ max: 256 },
	);
...
function runValidateOverlay({ env = process.env } = {}) {
	...
	if (env.OVERLAY)
		argv.push("--overlay", regularInputFile(env.OVERLAY, "OVERLAY"));
	if (env.INVENTORY)
		argv.push("--inventory", regularInputFile(env.INVENTORY, "INVENTORY"));
...
function runValidateOverlays() {
	return {
		ok: true,
		targets: SUPPORTED_TARGET_IDS.map((target) =>
			validateOverlayCommand(["--target", target, "--json"]),
		),
	};
}
...
export async function runReleaseInterface(command, { env = process.env } = {}) {
	...
	if (command === "current-target-admission")
		return runCurrentTargetAdmissionCommand({ env });
	if (command === "topology") return runTopology();
	if (command === "install") return runInstall({ env });
	fail(
		"E_RELEASE_INTERFACE_USAGE",
		`unknown release interface command: ${command}`,
		{ command },
	);
}
