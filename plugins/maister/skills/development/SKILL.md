...
The `AskUserQuestion` lines below describe the host adapter's
...
Host adapter presentation matrix (each `AskUserQuestion` entry is a presentation
primitive behind the engine; it never bypasses `evaluate_gate`):

- `phase-1-clarification`: AskUserQuestion
- `phase-1-exit`: AskUserQuestion
- `phase-2-scope-decision`: AskUserQuestion
- `phase-2-routing`: AskUserQuestion
- `phase-3-exit`: AskUserQuestion
- `phase-4-exit`: AskUserQuestion
- `requirements-clarification`: AskUserQuestion
- `phase-5-exit`: AskUserQuestion
- `optional-phase-selection/spec-audit`: AskUserQuestion
- `phase-6-exit`: AskUserQuestion
- `phase-7-exit`: AskUserQuestion
- `implementation-approval`: AskUserQuestion
- `phase-8-exit`: AskUserQuestion
- `phase-9-exit`: AskUserQuestion
- `verification-options`: AskUserQuestion
- `optional-phase-selection/e2e`: AskUserQuestion
- `optional-phase-selection/user-docs`: AskUserQuestion
- `verification-fix-selection`: AskUserQuestion
- `verification-rerun`: AskUserQuestion
- `verification-known-issues`: AskUserQuestion
- `phase-11-exit`: AskUserQuestion
- `phase-12-exit`: AskUserQuestion
- `phase-13-exit`: AskUserQuestion
- `final-handoff-approval`: AskUserQuestion
...
3. For each critical clarification, invoke the engine as `phase-1-clarification` with the exact question and the ordered options from the checklist; `AskUserQuestion` is only its user-gate adapter.
...
→ **MANDATORY GATE** — invoke the shared engine for `phase-2-routing` now; its user adapter is `AskUserQuestion`, and continuation requires the persisted terminal decision.
...
→ **MANDATORY GATE** — invoke the shared gate engine now; `AskUserQuestion` is only its user-gate adapter. Proceeding without a persisted terminal engine record is a protocol violation.
...
**Execute**: Direct - build the complete `verification-options` gate context and invoke the shared engine; `AskUserQuestion` is only the adapter.
