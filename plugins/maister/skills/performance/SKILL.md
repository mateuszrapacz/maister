...
The `AskUserQuestion` text below is the adapter's `present_user_gate`, never a
...
**Execute**: Direct - invoke the shared engine for the verification options gate; `AskUserQuestion` is only its adapter.
