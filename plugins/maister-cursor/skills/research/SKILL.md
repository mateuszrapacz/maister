...
2. For each decision area sequentially, output ALL of the following (steps a-d) BEFORE calling AskQuestion:
   ...
   d. **AskQuestion**: this area's alternatives as options (mark recommended with "(Recommended)") + "Need more info" option
...
> **SELF-CHECK before each AskQuestion**: Did you output the alternatives with pros/cons for THIS area? If you only showed a recommendation line without listing all alternatives and their pros/cons, STOP and output the full detail before asking.
