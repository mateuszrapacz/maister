...
The `AskQuestion` text below is the adapter's `present_user_gate`, never a
direct bypass. For each call site invoke `evaluate_gate(gate_context,
orchestrator-state.yml, host_adapter)` from
`lib/orchestrator-framework/references/gate-decision-engine.md` with
...
**Execute**: Direct - invoke the shared engine for the verification options gate; `AskQuestion` is only its adapter.
