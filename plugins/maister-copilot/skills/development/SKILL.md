...
The `ask_user` lines below describe the host adapter's
...
Host adapter presentation matrix (each `ask_user` entry is a presentation
primitive behind the engine; it never bypasses `evaluate_gate`):

- `phase-1-clarification`: ask_user
- `phase-1-exit`: ask_user
- `phase-2-scope-decision`: ask_user
- `phase-2-routing`: ask_user
- `phase-3-exit`: ask_user
- `phase-4-exit`: ask_user
- `requirements-clarification`: ask_user
- `phase-5-exit`: ask_user
- `optional-phase-selection/spec-audit`: ask_user
- `phase-6-exit`: ask_user
- `phase-7-exit`: ask_user
- `implementation-approval`: ask_user
- `phase-8-exit`: ask_user
- `phase-9-exit`: ask_user
- `verification-options`: ask_user
- `optional-phase-selection/e2e`: ask_user
- `optional-phase-selection/user-docs`: ask_user
- `verification-fix-selection`: ask_user
- `verification-rerun`: ask_user
- `verification-known-issues`: ask_user
- `phase-11-exit`: ask_user
- `phase-12-exit`: ask_user
- `phase-13-exit`: ask_user
- `final-handoff-approval`: ask_user
...
3. For each critical clarification, invoke the engine as `phase-1-clarification` with the exact question and the ordered options from the checklist; `ask_user` is only its user-gate adapter.
...
→ **MANDATORY GATE** — invoke the shared engine for `phase-2-routing` now; its user adapter is `ask_user`, and continuation requires the persisted terminal decision.
...
→ **MANDATORY GATE** — invoke the shared gate engine now; `ask_user` is only its user-gate adapter. Proceeding without a persisted terminal engine record is a protocol violation.
...
**Execute**: Direct - build the complete `verification-options` gate context and invoke the shared engine; `ask_user` is only the adapter.
...
**Q1** (always): invoke `verification-options` (sequential single-select) — "Which standard verifications to run?"
