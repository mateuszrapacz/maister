...
After forming a preliminary classification, **always present it to the user** via `ask_user` before proceeding. The user may know things that aren't visible in the code:
...
**Before diagnosing — ask about exceptions** via `ask_user`:
...
Tests can live at the aggregate level OR at the application service / facade level. Before recommending, **ask the user** via `ask_user`:
