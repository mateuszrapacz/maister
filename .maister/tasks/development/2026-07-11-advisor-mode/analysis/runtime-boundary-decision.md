...
The capability audit now supports a Markdown-first contract plus a bundled, host-neutral `phase-continue.mjs` adapter. The helper owns exact option validation, denylist checks, atomic state persistence, report generation, idempotency reuse, and phase transition; host runtimes only invoke it and continue from its normalized result.
...
|---|---|---|---|---|
| Cursor | `maister-advisor` Cursor subagent/Task | `AskQuestion` | `/maister-resume` | `phase-continue.mjs` → `phase_continue(selected_option)` |
| Kiro | `maister-advisor` JSON agent with read/grep/glob only | **CHAT GATE** | `@resume`/state read | `phase-continue.mjs` → `phase_continue(selected_option)` |
| Kilo | `@maister-advisor` with edit/bash denied | Chat gate | `.maister` state read | `phase-continue.mjs` → `phase_continue(selected_option)` |
| Copilot | Generated `advisor` agent | `ask_user` | State read by resume workflow | `phase-continue.mjs` → `phase_continue(selected_option)` |
| Codex | Native `advisor` role from `advisor.toml` | Plain-text user question | `$maister:resume` | `phase-continue.mjs` → `phase_continue(selected_option)` |
