...
const AGENT_FIXTURE_ROOT = path.join(ROOT, "tests/fixtures/platform-independent/agent-ir");
const AGENT_PROJECTION_PATH = path.join(ROOT, "plugins/maister/agent-projection-v1.json");
...
function readAgentFixture(relativePath) {
  ...
  return { path: `agents/${path.basename(relativePath)}`, source: fs.readFileSync(sourcePath, "utf8") };
}
...
test("parses the exact real 28-role inventory into immutable sorted Agent IR", () => {
  ...
  assert.deepEqual(ir.roles.map(({ role_id }) => role_id), EXPECTED_ROLE_IDS);
  assert.match(ir.canonical_set_digest, /^[0-9a-f]{64}$/u);
  assert.ok(Object.isFrozen(ir));
  assert.ok(Object.isFrozen(ir.roles));
  assert.ok(ir.roles.every((role) => Object.isFrozen(role) && Object.isFrozen(role.skill_dependencies)));
  assert.ok(ir.roles.every((role) => role.source_path === `agents/${role.role_id}.md`));
  assert.ok(ir.roles.every((role) => role.logical_role_id === `maister:${role.role_id}`));
  assert.ok(ir.roles.every((role) => role.instruction_body.trim().length > 0));
  assert.deepEqual(
    ir.roles.find(({ role_id }) => role_id === "docs-operator").skill_dependencies,
    ["docs-manager"],
  );
});
...
test("preserves complete instruction bodies and source-order skill dependencies", () => {
  ...
  assert.equal(role.instruction_body, "# Reviewer\n\nKeep every instruction byte.\n");
});
...
test("rejects YAML aliases, tags, and non-string metadata scalars", () => {
  const cases = ["invalid/yaml-alias.md", "invalid/yaml-tag.md", "invalid/scalar-description.md"];
...
test("rejects duplicate, case-folded, and normalized source identity collisions", () => {
  ...
  const collisions = [
    [{ path: "agents/reviewer.md", source: canonical }, { path: "agents/reviewer.md", source: canonical }],
    [{ path: "agents/reviewer.md", source: canonical }, { path: "agents/Reviewer.md", source: canonical.replace("name: reviewer", "name: Reviewer") }],
    [{ path: "agents/cafe\u0301.md", source: canonical.replace("name: reviewer", "name: cafe\u0301") }, { path: "agents/caf\u00e9.md", source: canonical.replace("name: reviewer", "name: caf\u00e9") }],
  ];
  for (const sources of collisions) {
    expectTypedError(
      () => parseAgentSources({ sources, skillsRoot: path.join(AGENT_FIXTURE_ROOT, "valid/skills") }),
      AgentIrValidationError,
...
test("loads a closed versioned projection contract with exact canonical and support inventories", () => {
  ...
  assert.deepEqual(contract.roles.map(({ role_id }) => role_id), EXPECTED_ROLE_IDS);
  assert.deepEqual(contract.support_inventory.map(({ support_id }) => support_id), [
    "cursor:explore",
    "kiro-cli:explore",
    "kiro-cli:maister",
  ]);
...
test("rejects unknown projection fields, unsupported profile IDs, and incomplete role coverage", () => {
  const mutations = [
    ...
    [
      (contract) => { contract.roles[1].execution_profiles.codex = "codex.unknown"; },
      "E_AGENT_MANIFEST_PROFILE",
    ],
    [
      (contract) => { contract.roles.pop(); },
...
test("builds a deterministic sorted manifest with exact target identities and profile bindings", () => {
  ...
  assert.deepEqual(first.rows, [...first.rows].sort((left, right) =>
  ...
  for (const roleId of EXPECTED_ROLE_IDS) {
    ...
    assert.equal(rows.find(({ target }) => target === "codex").adapter_id, "codex.exec");
    assert.equal(rows.find(({ target }) => target === "codex").native_role_external_id, null);
    for (const target of ["cursor", "kiro-cli"]) {
      assert.equal(rows.find((row) => row.target === target).native_role_external_id, `maister-${roleId}`);
    }
    assert.ok(rows.every((row) => row.logical_role_id === `maister:${roleId}`));
    assert.ok(rows.every((row) => row.source_sha256 === ir.roles.find((role) => role.role_id === roleId).source_sha256));
...
test("returns no manifest when IR, target, or profile validation fails", () => {
  ...
  expectTypedError(
    () => { manifest = buildAgentManifest({ agentIr: ir, projectionContract: contract, overlays }); },
    AgentManifestValidationError,
    "E_AGENT_MANIFEST_PROFILE",
  );
...
test("validates complete target projection tables and keeps support inventory separate", () => {
  for (const target of SUPPORTED_TARGET_IDS) {
    ...
    assert.deepEqual(overlay.agent_projection.canonical_roles, EXPECTED_ROLE_IDS);
    ...
    assert.ok(overlay.agent_projection.support_inventory.every(({ support_id }) =>
  ...
  assert.deepEqual(readFixture("cursor").agent_projection.support_inventory.map(({ support_id }) => support_id), ["explore"]);
  assert.deepEqual(readFixture("kiro-cli").agent_projection.support_inventory.map(({ support_id }) => support_id), ["explore", "maister"]);
});
...
test("rejects incomplete canonical coverage and unsafe projection or support destinations", () => {
  ...
  unsafeProjection.agent_projection.destinations[0].path_template = "../agents/{role_id}.md";
  expectOverlayError(() => validateOverlay(unsafeProjection), "E_OVERLAY_PATH");

  const unsafeSupport = readFixture("kiro-cli");
  unsafeSupport.agent_projection.support_inventory[0].assets[0].destination = "../agents/explore.json";
...
test("rejects duplicate execution profiles and target-to-adapter identity mismatches", () => {
  ...
  duplicate.agent_projection.execution_profile_ids.push(duplicate.agent_projection.execution_profile_ids[0]);
...
test("rejects unsupported overlay profile references during manifest construction", () => {
  ...
  expectTypedError(
    () => buildAgentManifest({ agentIr: ir, projectionContract: contract, overlays }),
    AgentManifestValidationError,
    "E_AGENT_MANIFEST_PROFILE",
  );
});
...
test("rejects every prohibited Advisor-only projection or execution exception", () => {
  const mutations = [
    (contract) => { contract.roles[0].readonly = true; },
    (contract) => { contract.roles[0].sandbox = "read-only"; },
    (contract) => { contract.roles[0].destination = "agents/advisor.md"; },
    (contract) => { contract.roles[0].permissions_profile_id = "advisor-only"; },
    (contract) => { contract.roles[0].model_profile_id = "advisor-only"; },
    (contract) => { contract.roles[0].adapter_id = "advisor.special"; },
    (contract) => { contract.roles[0].evidence_bypass = true; },
    (contract) => { contract.roles[0].lifecycle = "legacy"; },
    (contract) => { contract.support_inventory.push({ support_id: "codex:advisor", target: "codex", source: "advisor.toml", destination: ".codex/agents/advisor.toml", mode: "0644" }); },
  ];
  for (const mutate of mutations) {
    const contract = readProjectionContract();
    mutate(contract);
...
test("requires pinned Codex model and reasoning profiles while allowing no role-name conditionals", () => {
  const contract = loadAgentProjectionContract({ projectionPath: AGENT_PROJECTION_PATH });
  const codexProfiles = contract.profiles.execution.filter(({ target }) => target === "codex");

  assert.ok(codexProfiles.length > 0);
  for (const profile of codexProfiles) {
    const model = contract.profiles.model.find(({ id }) => id === profile.model_profile_id);
    const reasoning = contract.profiles.reasoning.find(({ id }) => id === profile.reasoning_profile_id);
...
test("keeps checked-in Cursor and Kiro agent assets limited to explicit support inputs", () => {
  const cursorAgentAssets = path.join(CHECKED_IN_OVERLAY_ROOT, "cursor/assets/agents");
  const kiroAgentAssets = path.join(CHECKED_IN_OVERLAY_ROOT, "kiro-cli/assets/agents");
  const cursorSupportAssets = path.join(CHECKED_IN_OVERLAY_ROOT, "cursor/assets/support-agents");
  const kiroSupportAssets = path.join(CHECKED_IN_OVERLAY_ROOT, "kiro-cli/assets/support-agents");

  const filesBelow = (root) => fs.existsSync(root)
    ? fs.readdirSync(root, { recursive: true }).filter((entry) => fs.statSync(path.join(root, entry)).isFile())
    : [];
  assert.deepEqual(filesBelow(cursorAgentAssets), []);
  assert.deepEqual(filesBelow(kiroAgentAssets), []);
  assert.deepEqual(
    fs.readdirSync(cursorSupportAssets, { recursive: true }).filter((entry) => fs.statSync(path.join(cursorSupportAssets, entry)).isFile()).sort(),
    ["explore.md"],
  );
  assert.deepEqual(
    fs.readdirSync(kiroSupportAssets, { recursive: true }).filter((entry) => fs.statSync(path.join(kiroSupportAssets, entry)).isFile()).sort(),
    [
