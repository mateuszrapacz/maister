...
function availableVersion(commandOrOutput, args) {
	const output =
		args === undefined ? (commandOrOutput ?? "Pi 0.80.10\n") : "Pi 0.80.10\n";
	return { status: 0, stdout: output, stderr: "", error: null, signal: null };
}

function environmentObservation(overrides = {}) {
	const nativeRoleExternalIds = manifest().rows.map(
		(row) => row.native_role_external_id,
	);
	return {
		...
		native_descriptors: nativeRoleExternalIds.map(
...
test("Pi probe is registered and passes only a supported executable range, Node, public export, and inventory tuple", () => {
	...
	assert.deepEqual(
		result.records.map((record) => record.result),
		["passed", "passed"],
	);
	assert.equal(
		result.records[0].provenance.executable_realpath,
		"/opt/pi/bin/pi",
	);
	assert.equal(
		result.records[0].provenance.node_version,
		PI_COMPATIBILITY.node,
	);
	assert.equal(
		result.records[1].provenance.prerequisite_version,
		PI_COMPATIBILITY.piSubagents,
	);
...
test("Pi probe accepts Pi 0.81.1 and rejects the exclusive range boundaries", () => {
	const current = probe({
		...
		discover: () =>
			environmentObservation({
				pi_version: "0.81.1",
				pi_engine_version: "0.81.1",
			}),
	});
	assert.deepEqual(
		current.records.map((record) => record.result),
		["passed", "passed"],
	);
	...
	const belowMinimum = probe({
		...
		discover: () =>
			environmentObservation({
				pi_version: "0.80.9",
				pi_engine_version: "0.80.9",
			}),
	});
	assert.deepEqual(
		belowMinimum.records.map((record) => record.provenance.reason),
	...
	const atMaximum = probe({
		...
		discover: () =>
			environmentObservation({
				pi_version: "0.82.0",
				pi_engine_version: "0.82.0",
			}),
	});
	assert.deepEqual(
		atMaximum.records.map((record) => record.provenance.reason),
		["pi_version_mismatch", "pi_version_mismatch"],
	);
});
...
test("Pi probe fails closed with typed unavailable reasons and does not infer native support from version alone", () => {
	...
	assert.equal(
		missingExport.records[0].provenance.reason,
		"public_export_missing",
	);
	assert.equal(
		missingExport.records[1].provenance.reason,
		"public_export_missing",
	);
	...
	assert.equal(
		versionOnly.records[0].provenance.reason,
		"delegation_package_missing",
	);
	...
	assert.deepEqual(
		missingRuntime.records.map((record) => record.provenance.reason),
	...
	assert.deepEqual(
		unsupportedPlatform.records.map((record) => record.provenance.reason),
	...
	assert.equal(
		missingDigest.records[0].provenance.reason,
		"delegation_package_missing",
	);
...
test("Pi discovery rejects public delegation exports with incompatible event channel values", () => {
	...
	assert.deepEqual(
		result.records.map((record) => record.provenance.reason),
		["public_event_mismatch", "public_event_mismatch"],
	);
});

test("Pi evidence expiry and host/prerequisite binding force renewal", () => {
	const baseline = createEvidenceRecord({
		target: "pi",
		capability: "E1",
		hostVersion: "0.80.10",
	...
	const native = createEvidenceRecord({
		...
		provenance: provenance({
			executable_realpath: "/opt/pi/bin/pi",
			node_version: "25.9.0",
			prerequisite_version: "0.35.1",
			prerequisite_digest: DIGEST_D,
			protocol_version: 1,
		}),
		timestamp: NOW,
	});
	assert.equal(
		Date.parse(baseline.expires_at) - Date.parse(NOW),
		EVIDENCE_EXPIRY_MS.baseline,
	);
	assert.equal(
		Date.parse(native.expires_at) - Date.parse(NOW),
		EVIDENCE_EXPIRY_MS.native,
	);
	assert.equal(
		evidenceNeedsRenewal(native, {
...
test("Pi claim evaluation allows provisional packaging but blocks native/semantic claims when E5/E6 are unavailable", () => {
	const records = collectEvidence({
		...
		provenance: provenance(),
		timestamp: NOW,
		records: ["E1", "E2", "E3", "E4"].map((capability) =>
			createEvidenceRecord({
				target: "pi",
				capability,
				hostVersion: "unknown",
